...
def check_dangerous_code(id, language):
    ...
    if language in ['python2', 'python3']:
        # code = file('/work/%s/main.py' % id).readlines()
        with open('%s/main.py' % work_path, 'r') as f:
            code = f.readlines()
        # print(code)
        support_modules = [
            're',  # 正则表达式
    ...
    if language in ['gcc', 'g++']:
        try:
            # code = file('/work/%s/main.c' % id).read()
            with open('%s/main.c' % work_path, 'r') as f:
                code = f.read()
        except:
            # code = file('/work/%s/main.cpp' % id).read()
            with open('%s/main.cpp' % work_path, 'r') as f:
                code = f.read()
    ...
    #        return True
    if language == 'go':
        # code = file('/work/%s/main.go' % id).read()
        with open('%s/main.go' % work_path, 'r') as f:
            code = f.read()
        danger_package = [
...
#     a = check_dangerous_code(2, 'python3')
#     print(a)
